...
{
    public class RealizarLoginEmailCommandTest : AutenticacaoCommandTestBase
    {
        #region Validações comando
        [Fact]
        public void Deve_Validar_Comando_Valido()
        {
            var comando = new RealizarLoginEmailCommandBuilder().ComandoValido().Build();
        ...
        {
            var comando = new RealizarLoginEmailCommandBuilder().ComandoInvalido().Build();
        ...
        [Fact]
        public async Task Deve_Nao_Encontrar_Usuario_Com_Email()
        {
            var comando = new RealizarLoginEmailCommandBuilder().ComandoValido().Build();

            _usuarioRepository.Setup(x => x.ExisteUsuarioComEmail(It.IsAny<string>())).Returns(false);

            var resultado = await _handler.Handle(comando, _cancellationToken);

            _mediatorHandler.Verify(x => x.PublicarNotificacao(It.Is<DomainNotification>(x => x.Value == "Não existe nenhum usuário cadastrado com esse e-mail")), Times.Once);

            Assert.Null(resultado);
        }

        [Fact]
        public async Task Deve_Nao_Encontrar_Usuario_Com_Email_Senha()
        {
            var comando = new RealizarLoginEmailCommandBuilder().ComandoValido().Build();

            _usuarioRepository.Setup(x => x.ExisteUsuarioComEmail(It.IsAny<string>())).Returns(true);
            _usuarioRepository.Setup(x => x.RealizarLoginEmail(It.IsAny<string>(), It.IsAny<string>())).Returns(default(Usuario));

            var resultado = await _handler.Handle(comando, _cancellationToken);

            _mediatorHandler.Verify(x => x.PublicarNotificacao(It.Is<DomainNotification>(x => x.Value == "E-mail e/ou senha inválidos")), Times.Once);
        ...
        public async Task Deve_Validar_Status_Conta_Usuario_Aguardando_Confirmacao()
        {
            ...
            _usuarioRepository.Setup(x => x.ExisteUsuarioComEmail(It.IsAny<string>())).Returns(true);
            _usuarioRepository.Setup(x => x.RealizarLoginEmail(It.IsAny<string>(), It.IsAny<string>())).Returns(usuario);

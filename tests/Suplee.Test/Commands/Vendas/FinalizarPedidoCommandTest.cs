...
{
    public class FinalizarPedidoCommandTest : VendasCommandTestBase
    {
        #region Validações comando
        [Fact]
        public void Deve_Validar_Comando_Valido()
        {
            var comando = new FinalizarPedidoCommandBuilder().ComandoValido().Build();
        ...
        {
            var comando = new FinalizarPedidoCommandBuilder().ComandoInvalido().Build();
        ...
        [Fact]
        public async Task Deve_Finalizar_Pedido_Com_Sucesso()
        {
            var usuarioId = Guid.NewGuid();

            var pedido = new PedidoBuilder().PadraoValido().ComUsuarioId(usuarioId).Build();

            var comando = new FinalizarPedidoCommandBuilder().ComandoValido().ComUsuarioId(usuarioId).ComPedidoId(pedido.Id).Build();
